export interface JioPayOpenHostedCheckoutOptions {
   ...
   * On Android, once the checkout page navigates to a URL starting with this
   * prefix, the native checkout screen closes and the Promise resolves with
   * that URL and its parsed query params. Ignored on Web, where the page
   * just does a full redirect and there's no separate "closed" step to
